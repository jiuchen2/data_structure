...
Flower::Flower(const Vector2& center)
  : stem(Vector2(center.x(), center.y() + STEM_HEIGHT / 2), STEM_COLOR, STEM_WIDTH, STEM_HEIGHT),
    ...
    leaf(LEAF_COLOR, Vector2(Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).x(), Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).y() - STEM_HEIGHT / 4), Vector2(Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).x() - LEAF_WIDTH, Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).y() - LEAF_HEIGHT), Vector2(Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).x() - LEAF_WIDTH / 2, Vector2(stem.center().x() - STEM_WIDTH / 2 + 1, stem.center().y() + STEM_HEIGHT / 4).y()))
{
}
