...
class KDTree
{
  ...
  private:
    ...
    /** Partition function in quickselect */
    unsigned partition(unsigned left, unsigned right, unsigned pivotIndex, int d);

    /** quickselect function */
    void quickselect(unsigned left, unsigned right, unsigned k, int d);

    /** KDTree constructor helper function */
    void buildTree(unsigned left, unsigned right, int d);

    /** findNearestNeighbor helper function */
    void findHelper(const Point<Dim>& target,Point<Dim> & best, double & radius, unsigned left, unsigned right, int d) const;

    /** calculate radius square */
    double getRadius(const Point<Dim> & a, const Point<Dim> & b) const;
