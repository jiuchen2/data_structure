...
void TileImage::paste(PNG& canvas, std::vector<int> startX, std::vector<int> startY, std::vector<int> resolutions) const {
      ...
      if (getResolution() % resolution == 0) {
          ...
          for (int x = 0; x < resolution; x++) {
              for (int y = 0; y < resolution; y++) {
                  ...
                  HSLAPixel pix = getScaledPixelInt(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
                  for (unsigned i = 0; i < startX.size(); i++) {
                    *(canvas.getPixel(startX[i] + x, startY[i] + y)) = pix;
                  }
                  // *(canvas.getPixel(startX[i] + x, startY[i] + y)) =
